...
import { Clock, Users, Package, CheckCircle, Search, X, ChevronRight, Sparkles } from "lucide-react";
...
const MinhasReservas = () => {
  ...
  const getReservasFiltradas = () => {
    ...
    switch (filtroStatus) {
      case 'ativas': reservasFiltradas = minhasReservasAtivas; break;
      case 'vendas': reservasFiltradas = meusItensReservados; break;
      case 'concluidas': reservasFiltradas = reservasConcluidas; break;
      default: return [];
    }
      ...
      <main className="container flex flex-col items-center justify-center mx-auto pt-32 pb-24 px-4 max-w-6xl">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tight">
            Minhas <span className="text-glow text-primary italic">Reservas</span>
          </h1>
          <p className="text-foreground/40 font-bold ml-8 uppercase tracking-widest text-[10px] mt-2">Acompanhe suas trocas e vendas em tempo real</p>
        </div>
            ...
            <div className="premium-card bg-white/70 backdrop-blur-2xl border-black/20 rounded-[3.5rem] p-10 shadow-[0_32px_64px_-16px_rgba(235,51,148,0.1)]">
                ...
                {stats.map((stat) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      stat.bg, stat.color,
                      filtroStatus === stat.filtro ? "scale-110 rotate-3 shadow-lg" : "group-hover:scale-110"
                    )}>
                      <stat.icon className="w-7 h-7" />
                    </div>
                    <div className="space-y-1">
                      <p className="text-4xl font-black text-foreground tracking-tighter drop-shadow-sm font-sans">{stat.value}</p>
                      <p className="text-[10px] font-black text-foreground/30 uppercase tracking-[0.2em] leading-none">{stat.title}</p>
                    </div>
              ...
              <div className="mt-10 ml-24 pt-10 border-t border-primary/5">
                ...
                <div className="flex items-center gap-2 mt-4 ml-6">
                  ...
                  <p className="text-[9px] font-black text-foreground/20 uppercase tracking-[0.2em]">O código único de 5 dígitos está na página do produto</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {codigoBusca ? (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-black text-xs italic italic">!</div>
                <h3 className="font-black text-foreground tracking-tight uppercase text-sm">Resultados para "GRM-{codigoBusca}"</h3>
              </div>
              ...
              {getReservasFiltradas().length === 0 && getFilasFiltradas().length === 0 && (
                <div className="premium-card bg-white/40 border-dashed border-primary/20 rounded-[3rem] py-20 text-center space-y-4">
                  ...
                  <p className="text-foreground/40 font-black uppercase tracking-widest text-xs">Nenhum registro encontrado</p>
                </div>
              )}
            </div>
          ) : filtroStatus !== 'todas' ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {filtroStatus === 'fila' ? (
                  getFilasFiltradas().map(fila => (
                    <FilaEsperaCard key={fila.id} fila={fila} onSairDaFila={sairDaFila} />
                  ))
                ) : (
                  getReservasFiltradas().map(reserva => (
                    <ReservaCard
            ...
            <div className="space-y-16">
                ...
                <section className="space-y-8">
                  <div className="flex items-center justify-between border-b border-primary/5 pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-2xl font-black text-foreground tracking-tight italic">Suas <span className="text-primary">Reservas</span></h2>
                    </div>
                    <Badge className="bg-primary/5 text-primary border-none rounded-full px-4 py-1 font-black">{minhasReservasAtivas.length}</Badge>
                  </div>
                  <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    {minhasReservasAtivas.map(reserva => (
                      <ReservaCard key={reserva.id} reserva={reserva} onConfirmarEntrega={confirmarEntrega} onCancelarReserva={cancelarReserva} onRefresh={refetch} onVerDetalhes={(itemId) => abrirModalItem(itemId, 'reserva')} />
                    ))}
                  </div>
                </section>
              )}
                ...
                <section className="space-y-8">
                  <div className="flex items-center justify-between border-b border-primary/5 pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-2xl font-black text-foreground tracking-tight italic">Filas de <span className="text-blue-500">Espera</span></h2>
                    </div>
                    <Badge className="bg-blue-50 text-blue-600 border-none rounded-full px-4 py-1 font-black">{filasEspera.length}</Badge>
                  </div>
                  <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                ...
                <section className="space-y-8">
                  <div className="flex items-center justify-between border-b border-primary/5 pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-2xl font-black text-foreground tracking-tight italic">Meus <span className="text-orange-500">Desapegos</span></h2>
                    </div>
                    <Badge className="bg-orange-50 text-orange-600 border-none rounded-full px-4 py-1 font-black">{meusItensReservados.length}</Badge>
                  </div>
                  <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    {meusItensReservados.map(reserva => (
                      <ReservaCard key={reserva.id} reserva={reserva} onConfirmarEntrega={confirmarEntrega} onCancelarReserva={cancelarReserva} onRefresh={refetch} onVerDetalhes={(itemId) => abrirModalItem(itemId, 'venda')} />
                    ))}
                  </div>
                </section>
              )}
                ...
                <section className="space-y-8">
                  <div className="flex items-center justify-between border-b border-primary/5 pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-2xl font-black text-foreground tracking-tight italic">Trocas <span className="text-emerald-500">Concluídas</span></h2>
                    </div>
                    <Badge className="bg-emerald-50 text-emerald-600 border-none rounded-full px-4 py-1 font-black">{reservasConcluidas.length}</Badge>
                  </div>
                  <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    {reservasConcluidas.slice(0, 6).map(reserva => (
                      <ReservaCard key={reserva.id} reserva={reserva} onConfirmarEntrega={confirmarEntrega} onCancelarReserva={cancelarReserva} onRefresh={refetch} onVerDetalhes={(itemId) => abrirModalItem(itemId, 'concluida')} />
                    ))}
                  </div>
                  {reservasConcluidas.length > 6 && (
                    <div className="text-center">
                      <Button onClick={() => setFiltroStatus('concluidas')} variant="ghost" className="text-[10px] font-black uppercase tracking-widest text-foreground/30 hover:text-primary transition-colors flex items-center mx-auto gap-2">
              ...
              {reservas.length === 0 && filasEspera.length === 0 && (
                <div className="premium-card bg-white/40 border-dashed border-primary/20 rounded-[3rem] py-32 text-center space-y-8">
                  <div className="w-24 h-24 bg-primary/5 rounded-full flex items-center justify-center mx-auto shadow-inner relative group">
                    
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-2xl font-black text-foreground tracking-tight italic">O coração da rede está batendo!</h3>
                    <p className="text-sm font-medium text-foreground/40 max-w-xs mx-auto leading-relaxed">Você ainda não tem reservas ativas. Que tal explorar o feed e encontrar o próximo tesouro para seu pequeno?</p>
                  </div>
                  <Button onClick={() => navigate('/feed')} className="founders-button px-10 h-14 text-white rounded-full">Explorar Feed</Button>
                </div>
              )}
            </div>
          )}
