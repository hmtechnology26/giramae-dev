    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 glass border-t border-primary/5 z-50 backdrop-blur-2xl transition-all h-[70px]">
      <div className="grid grid-cols-4 h-full items-center">
          ...
          return (
            ...
            >
              <div className="relative">
                <Icon className={cn("h-5 w-5 transition-transform", isActive && "text-glow drop-shadow-[0_0_10px_hsl(var(--primary)/0.3)]")} />
                {item.path === '/missoes' && missoesCompletas > 0 && (
                  <Badge className="absolute -top-2 -right-2 h-4 w-4 text-[8px] p-0 bg-primary border-2 border-white flex items-center justify-center font-bold text-white shadow-sm">
                    {missoesCompletas}
                  </Badge>
                )}
              </div>
              <span className={cn("text-[8px] font-bold tracking-[0.08em] uppercase", isActive ? "opacity-100" : "opacity-60")}>
                {item.label}
              </span>
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-primary/40 rounded-full blur-[1px]" />
              )}
            </Link>
          );
        })}
        ...
        <a
          ...
          className="flex flex-col items-center justify-center space-y-1 text-muted-foreground/40 hover:text-primary transition-all duration-300 h-full group"
        >
          <div className="relative">
            <MessageCircle className="h-5 w-5 group-hover:scale-105 transition-transform" />
          </div>
          <span className="text-[8px] font-bold tracking-[0.08em] uppercase opacity-60">Suporte</span>
        </a>
      </div>
    </nav>
  );
