-- REORGANIZAÇÃO: Integrar Penalidades no Gerenciamento de Usuários (CORRIGIDO)
...
SELECT 
    ...
    COALESCE(res_stats.total_reservas, 0) as total_reservas_feitas,
    COALESCE(stats.total_vendas, 0) as total_vendas_realizadas,
    COALESCE(den_stats.total_denuncias, 0) as total_denuncias_feitas,
    ...
    -- Penalidades ativas
    COALESCE(pen.penalidades_ativas, 0) as penalidades_ativas,
    COALESCE(pen.penalidade_mais_grave, 0) as penalidade_mais_grave,
    COALESCE(pen.total_penalidades_historico, 0) as total_penalidades_historico,
    pen.ultima_penalidade,
    
    -- Status derivado
    CASE 
        WHEN COALESCE(pen.penalidade_mais_grave, 0) >= 3 THEN 'suspenso'
        WHEN COALESCE(pen.penalidade_mais_grave, 0) >= 2 THEN 'warned'
        WHEN COALESCE(pen.penalidade_mais_grave, 0) = 1 THEN 'warned'
...
-- Estatísticas de atividade dos itens
LEFT JOIN (
    SELECT 
