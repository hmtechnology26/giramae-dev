-- Modificar função cancelar_reserva para tratar diferentes motivos de cancelamento
CREATE OR REPLACE FUNCTION public.cancelar_reserva(p_reserva_id uuid, p_usuario_id uuid, p_motivo text DEFAULT NULL::text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    -- ✅ NOVA LÓGICA CONDICIONAL BASEADA NO MOTIVO
    IF p_motivo = 'remover_item' THEN
        -- Opção 1: Remover item da plataforma
        UPDATE itens 
        SET status = 'inativo' 
        WHERE id = v_reserva.item_id;
        
        -- Limpar fila de espera (item não existe mais)
        DELETE FROM fila_espera 
        WHERE item_id = v_reserva.item_id;
        
    ELSE
        -- Opção 2 e 3: Manter item disponível
        UPDATE itens 
        SET status = 'disponivel' 
        WHERE id = v_reserva.item_id;
        
        -- Processar próximo da fila automaticamente
