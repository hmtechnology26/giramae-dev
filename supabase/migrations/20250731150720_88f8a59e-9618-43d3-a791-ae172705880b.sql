-- Corrigir constraint e função para usar códigos corretos

-- 1. Primeiro, atualizar a constraint para aceitar os códigos reais da tabela motivos_cancelamento
ALTER TABLE reservas DROP CONSTRAINT IF EXISTS check_motivo_cancelamento;

ALTER TABLE reservas ADD CONSTRAINT check_motivo_cancelamento 
CHECK (
  motivo_cancelamento IS NULL OR 
  motivo_cancelamento IN (
    'remover_item',
    'trocar_comprador', 
    'comprador_desistencia',
    'outro',
    'sistema_expiracao'
  )
);

-- 2. Atualizar função cancelar_reserva_v2 para usar códigos corretos
...
BEGIN
  ...
  CASE p_motivo_codigo
    ...
    WHEN 'comprador_desistencia' THEN
      v_novo_status_item := 'disponivel';
    WHEN 'outro' THEN
      v_novo_status_item := 'disponivel';
    WHEN 'sistema_expiracao' THEN
      v_novo_status_item := 'disponivel';
    ELSE
  ...
  -- 🔄 RESERVA: Cancelar reserva com código correto
  UPDATE reservas 
  SET status = 'cancelada',
      motivo_cancelamento = p_motivo_codigo,  -- ✅ AGORA USA O CÓDIGO DIRETAMENTE
      observacoes_cancelamento = p_observacoes,
      cancelada_em = now(),
      cancelada_por = p_usuario_id
  WHERE id = p_reserva_id;
  
  -- 📊 MÉTRICAS: Verificar se tabela existe antes de inserir
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reservation_cancel_metrics') THEN
