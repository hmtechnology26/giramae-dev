-- Correção: Drop da função existente e recriação com correções críticas

-- 1. Remover função existente para permitir alteração do tipo de retorno
DROP FUNCTION IF EXISTS processar_proximo_fila(UUID);

-- 2. Criar tabela de motivos de cancelamento se não existir
...
-- 4. Criar tabela de fila de processamento para controle robusto
...
-- 5. Criar índices de performance para fila
...
-- 6. Criar tabela de dead letter queue para falhas
...
-- 7. Recriar função processar_proximo_fila com controle de concorrência
...
BEGIN
    ...
    IF FOUND THEN
        ...
        -- Buscar valor do item e criar nova reserva
        WITH item_data AS (
...
-- 8. Criar função melhorada para cancelar reserva com diferentes motivos
...
BEGIN
    ...
    -- Registrar o cancelamento na fila de processamento
    INSERT INTO fila_processamento (reserva_id, item_id, tipo_operacao, metadados)
